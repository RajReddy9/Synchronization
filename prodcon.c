...
void *producer(void *contextPtr)
{
	...
	//printf("msg_queue_write ret is %d\n", ret);
	//printf("msg_queue_write count is %d\n", count);
	assert(ret == 0);
	(void)ret;
	...
	for (int i = 0; i < count; i++) { 
		//printf("----Producer for loop number %d----\n",i);
		int number = rand() % 64;
		...
		sum += number;
		//printf("write sum is %d\n", sum);
		//printf("----End producer for loop----\n");
	}
	context->sum = sum;
...
int main(int argc, char *argv[])
{
	...
	//printf("msg_queue_read size len is %lu\n",len);
